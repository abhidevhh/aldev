...
test(
	'abhi-call-audio-cli stitches and uploads episode audio with local fixtures',
	async () => {
	const tempRoot = await fs.mkdtemp(
		path.join(os.tmpdir(), 'abhi-call-audio-cli-'),
	)
	...
	try {
		...
		const scriptPath = path.join(
			process.cwd(),
			'sandbox',
			'abhi-call-audio-cli.sh',
		)

		const { stdout, stderr } = await runCommand('bash', [scriptPath], {
			ABHI_CALL_AUDIO_DRAFT_ID: 'draft-123',
			ABHI_CALL_AUDIO_ATTEMPT: '1',
			ABHI_CALL_AUDIO_ASSETS_DIR: assetsDir,
...
test('abhi-call-audio-cli fails fast when a required env var is missing', async () => {
	const scriptPath = path.join(
		process.cwd(),
		'sandbox',
		'abhi-call-audio-cli.sh',
	...
	await expect(
		runCommand('bash', [scriptPath], {
			ABHI_CALL_AUDIO_ATTEMPT: '1',
			ABHI_CALL_AUDIO_ASSETS_DIR: '/tmp/does-not-matter',
	...
	).rejects.toThrow(/Missing required env var: ABHI_CALL_AUDIO_DRAFT_ID/)
})

test('abhi-call-audio-cli fails when an input download returns 404', async () => {
	const tempRoot = await fs.mkdtemp(
		path.join(os.tmpdir(), 'abhi-call-audio-cli-missing-download-'),
	)
	tempPaths.push(tempRoot)
	...
	try {
		...
		await expect(
			runCommand('bash', [scriptPath], {
				ABHI_CALL_AUDIO_DRAFT_ID: 'draft-404',
				ABHI_CALL_AUDIO_ATTEMPT: '1',
				ABHI_CALL_AUDIO_ASSETS_DIR: assetsDir,
				CALL_AUDIO_URL: `${baseUrl}/call.mp3`,
				RESPONSE_AUDIO_URL: `${baseUrl}/response.mp3`,
