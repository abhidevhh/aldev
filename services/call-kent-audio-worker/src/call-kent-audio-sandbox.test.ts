...
import {
	...
	runAbhiCallAudioSandboxJob,
} from './abhi-call-audio-sandbox.ts'
...
test('runAbhiCallAudioSandboxJob executes the sandbox CLI and destroys the sandbox', async () => {
	...
	const result = await runAbhiCallAudioSandboxJob({
		binding: {} as never,
		sandboxId: 'sandbox-1',
		request: {
			draftId: 'draft-123',
	...
	expect(exec).toHaveBeenCalledWith('/usr/local/bin/abhi-call-audio-cli', {
...
test('runAbhiCallAudioSandboxJob surfaces sandbox stderr on failure', async () => {
	const destroy = vi.fn().mockResolvedValue(undefined)
...
test('runAbhiCallAudioSandboxJob retries transient sandbox startup errors', async () => {
	const exec = vi
		.fn()
