...
var Gravity = __assign2({ Auto: "auto", Subject: "auto:subject", Face: "face", Sink: "sink", FaceCenter: "face:center", MultipleFaces: "faces", MultipleFacesCenter: "faces:center" }, Compass);
...
var computeCondition = function(conditionObj) {
  var expression = conditionObj.expression ? toString(conditionObj.expression.map(function(exp) {
    return computeConditionExpression(exp);
  }), "_and_") : "";
  var transformations = conditionObj.transformations.map(function(transformation) {
    return toTransformationStr(transformers_default(transformation));
  });
  return {
    expression,
...
var mapCharacteristic = function(expression) {
  return ConditionalParams[expression] || expression;
};
...
var mapArithmeticExpression = function(orgExpression) {
  ...
  return orgExpression.replace(regex, function(match) {
    return ArithmeticExpression[match];
  });
};
var mapPredefinedVariables = function(orgExpression) {
  ...
  return orgExpression.replace(regex, function(match, p, index) {
    return orgExpression[index - 1] === Prefix ? match : PredefinedVariables[match];
  });
};
...
var variables = function(value) {
  ...
  return toString(value.map(function(variable) {
...
var transformers_default = transform;
...
var getPrefix = function(publicId, _a) {
  ...
  var subDomain = hasSecureDistribution ? "" : getSubDomain(publicId, { cdnSubdomain, cname });
...
var url = function(publicId, cloud, options) {
  ...
  var $options = __assign3(__assign3({ quality: "auto" }, options), { format });
...
var buildVideoUrl = function(publicId, _a) {
  var _b = _a.cloud, cloud = _b === void 0 ? {} : _b, _c = _a.transformations, transformations = _c === void 0 ? {} : _c;
  return buildUrl(publicId, {
    cloud: __assign4(__assign4({}, cloud), { resourceType: RESOURCE_TYPES.VIDEO }),
    transformations
  });
};
var Transformer = { transform, toString: toTransformationStr };
var esm_default = buildUrl;
export {
  Transformer,
  buildImageUrl,
  buildUrl,
...
//# sourceMappingURL=cloudinary-build-url.js.map
