...
import { getEpisodePath as getAbhiCallEpisodePath } from '#app/utils/abhi-call.ts'
import { getAbhiTalksEpisodePath } from '#app/utils/abhi-talks.ts'
...
async function fetchSimplecastEpisodes() {
	...
	const PODCAST_ID = getRequiredEnv('ABHI_TALKS_PODCAST_ID')
...
async function main() {
	...
	console.log(`abhi call episodes: ${ckEpisodes.length}`)
	console.log(`abhi talks episodes: ${cwkEpisodes.length}`)
	...
	for (const e of ckEpisodes) {
		...
		const url = getAbhiCallEpisodePath({
	...
	for (const e of cwkEpisodes) {
		...
		if (isIgnoredDocId(docId)) continue
		const url = getAbhiTalksEpisodePath({ seasonNumber, episodeNumber, slug: e.slug })
