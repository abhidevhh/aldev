...
function getEpisodeDraftAudioKey({
	...
	return `abhi-call/drafts/${draftId}/${episodeDraftAudioFileNameByKind[kind]}`
}

export type AbhiCallAudioSignedUrls = {
	callAudioUrl: string
...
}): Promise<AbhiCallAudioSignedUrls> {
		...
		await Promise.all([
			getSignedUrl(
				...
				new GetObjectCommand({
					Bucket: env.ABHI_CALL_R2_BUCKET,
					Key: callAudioKey,
				}),
