...
outside the repo, set `REPO_ROOT=/path/to/abhidev.com` when running it.
...
```sh
REPO_ROOT=/volume1/code/abhidev.com \
other/nas/youtube/start-youtube-indexer.sh
```
...
repo under `/volume1/docker/abhidev.com/repo`, then runs
`start-youtube-indexer.sh` with `REPO_ROOT` set.

For a simple cron line without the bundled task script:

```sh
cd /path/to/abhidev.com && other/nas/youtube/start-youtube-indexer.sh
```

If you already have a full repo checkout on the NAS and only need a pull:

```sh
cd /path/to/abhidev.com && git pull --ff-only && other/nas/youtube/start-youtube-indexer.sh
```
