...
This is AbhiDev' personal website (abhidev.com) — a React Router v7 app
...
| DB reset + seed | `npm exec --workspace abhidev.com prisma migrate reset --force`                |
...
- abhi call FFmpeg offload caveat: episode audio generation runs through a
  ...
  `services/abhi-call-audio-worker/assets/{intro,interstitial,outro}.mp3`.

## Seed data

The seed script (`services/site/prisma/seed.ts`) creates an admin user:
`me@abhidev.com` / `iliketwix` (role ADMIN, Blue Team).
...
- The Cursor Cloud VM snapshot ships with Node 24 via nvm, Chrome configured to
  ...
  as the seed admin user (`me@abhidev.com` / `iliketwix`).
