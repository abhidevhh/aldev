...
export type AbhiCallAudioSandboxRequest = {
	draftId: string
	attempt: number
	callAudioUrl: string
...
export function createSandboxCommandEnvironment(
	request: AbhiCallAudioSandboxRequest,
) {
	return {
		ABHI_CALL_AUDIO_DRAFT_ID: request.draftId,
		ABHI_CALL_AUDIO_ATTEMPT: String(request.attempt),
...
export async function runAbhiCallAudioSandboxJob({
	binding,
	sandboxId,
	request,
	...
	try {
		...
		for (let attempt = 1; attempt <= sandboxStartupRetryCount; attempt++) {
			try {
				result = await sandbox.exec('/usr/local/bin/abhi-call-audio-cli', {
		...
		await sandbox.destroy().catch((error: unknown) => {
			console.warn('Failed to destroy abhi call audio sandbox', {
				draftId: request.draftId,
